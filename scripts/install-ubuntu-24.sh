...
WEB_PUSH_CONTACT=mailto:admin@${DOMAIN}
...
cat > "/etc/systemd/system/${APP_NAME}-live-chat.service" <<EOF
[Unit]
Description=Clipeshare live chat / presence server
After=network.target mysql.service
...
WorkingDirectory=${APP_DIR}/current
EnvironmentFile=${APP_DIR}/shared/.env.production
ExecStart=/usr/bin/npm run live-chat
Restart=always
RestartSec=5
NoNewPrivileges=true
PrivateTmp=true
