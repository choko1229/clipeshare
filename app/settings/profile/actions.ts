...
const profileSchema = z.object({
  ...
  profileOverlayColor: z.string().trim().regex(/^#[0-9a-fA-F]{6}$/).optional().or(z.literal("")),
  profileOverlayColorEnd: z.string().trim().regex(/^#[0-9a-fA-F]{6}$/).optional().or(z.literal("")),
  profileOverlayOpacity: z.coerce.number().int().min(0).max(100),
  profileThemePreference: z.enum(["SYSTEM", "DARK", "LIGHT"]),
  discordAutoMirrorEnabled: z.boolean(),
...
async function updateProfileInternal(formData: FormData) {
  ...
  const parsed = profileSchema.parse({
    ...
    profileOverlayColor: formData.get("profileOverlayColor") ?? "",
    profileOverlayColorEnd: formData.get("profileOverlayColorEnd") ?? "",
    profileOverlayOpacity: formData.get("profileOverlayOpacity") ?? 70,
    profileThemePreference: formData.get("profileThemePreference") ?? "SYSTEM",
    discordAutoMirrorEnabled: formData.get("discordAutoMirrorEnabled") === "on",
  ...
  await prisma.$transaction(async (tx) => {
    await tx.user.update({
      ...
      data: {
        ...
        profileOverlayColor: parsed.profileOverlayColor || null,
        profileOverlayColorEnd: parsed.profileOverlayColorEnd || null,
        profileOverlayOpacity: parsed.profileOverlayOpacity,
        profileThemePreference: parsed.profileThemePreference,
        discordAutoMirrorEnabled: parsed.discordAutoMirrorEnabled,
